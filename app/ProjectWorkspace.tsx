...
type ShapeKind = "arrow"|"line"|"rectangle"|"circle";
type CanvasNode = Node<CanvasNodeData,"note"|"image"|"response"|"doodle"|"group"|"shape">;
...
const buildCanvasSvg = (project:ProjectWorkspaceProject,nodes:CanvasNode[],edges:Edge[]) => {
  ...
  const nodeMarkup=renderNodes.map(({node,position,width:nodeW,height:nodeH})=>{const point=offset(position),title=escapeXml(node.data.title||"未命名卡片"),color=node.data.groupColor||"#d5a900",headerHeight=node.type==="group"?38:34;
    ...
    const bodyFontSize=Math.max(10,Math.min(48,Number(node.data.fontSize)||12));
    const body=svgLines(node.data.text||"").map((line,index)=>`<text x="${point.x+14}" y="${point.y+headerHeight+24+index*(bodyFontSize+5)}" fill="#252826" font-family="Arial, sans-serif" font-size="${bodyFontSize}">${escapeXml(line)}</text>`).join("");
    ...
    const shapeColor=escapeXml(node.data.shapeColor||"#d5a900"),shapeStrokeWidth=Math.max(1,Math.min(12,Number(node.data.shapeStrokeWidth)||3)),shapePadding=node.type==="shape"?2:12,shapeTopOffset=node.type==="shape"?2:headerHeight+12,shapeX=point.x+shapePadding,shapeY=point.y+shapeTopOffset,shapeW=Math.max(24,nodeW-shapePadding*2),shapeH=Math.max(24,nodeH-(node.type==="shape"?4:headerHeight+24)),shapeCenterY=shapeY+shapeH/2,arrowLineEnd=shapeX+shapeW*.82,arrowBaseX=shapeX+shapeW*.76,arrowTipX=shapeX+shapeW*.94,arrowHalfHeight=shapeH*.16,shapeMarkup=node.type==="shape"?(node.data.shapeKind==="line"?`<line x1="${shapeX+shapeW*.08}" y1="${shapeCenterY}" x2="${shapeX+shapeW*.92}" y2="${shapeCenterY}" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:node.data.shapeKind==="rectangle"?`<rect x="${shapeX+shapeW*.1}" y="${shapeY+shapeH*.15}" width="${shapeW*.8}" height="${shapeH*.7}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:node.data.shapeKind==="circle"?`<ellipse cx="${shapeX+shapeW/2}" cy="${shapeCenterY}" rx="${shapeW*.39}" ry="${shapeH*.35}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:`<line x1="${shapeX+shapeW*.08}" y1="${shapeCenterY}" x2="${arrowLineEnd}" y2="${shapeCenterY}" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/><path d="M ${arrowBaseX} ${shapeCenterY-arrowHalfHeight} L ${arrowTipX} ${shapeCenterY} L ${arrowBaseX} ${shapeCenterY+arrowHalfHeight}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}" stroke-linecap="square" stroke-linejoin="miter"/>`):"";
    if(node.type==="shape")return`<g aria-label="${title}">${shapeMarkup}</g>`;
...
function NoteNode({id,selected,data}:NodeProps<CanvasNode>) {
  const fontSize=Math.max(10,Math.min(48,Number(data.fontSize)||14));
  return <NodeShell id={id} selected={selected} data={data}><textarea className="nodrag" style={{fontSize}} value={data.text || ""} onChange={event=>data.update?.(id,{text:event.target.value})} placeholder="输入文字..."/></NodeShell>;
}
...
const shapeLabels:Record<ShapeKind,string>={arrow:"箭头",line:"直线",rectangle:"矩形",circle:"圆形"};
function ShapeNode({selected,data}:NodeProps<CanvasNode>) {
  const kind=data.shapeKind||"arrow",color=data.shapeColor||"#d5a900",strokeWidth=Math.max(1,Math.min(12,Number(data.shapeStrokeWidth)||3));
  return <div className={`project-pure-shape-node ${selected?"selected":""}`} aria-label={`${shapeLabels[kind]}图案`}>
    <NodeResizer isVisible={selected} minWidth={64} minHeight={40} lineStyle={{borderColor:"#202322"}} handleStyle={{width:8,height:8,border:"1px solid #202322",background:"#f2d600"}} />
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      {kind==="line"&&<line x1="8" y1="50" x2="92" y2="50" stroke={color} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke"/>}
      {kind==="arrow"&&<><line x1="8" y1="50" x2="82" y2="50" stroke={color} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke"/><path d="M 76 34 L 94 50 L 76 66" fill="none" stroke={color} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke" strokeLinecap="square" strokeLinejoin="miter"/></>}
      {kind==="rectangle"&&<rect x="10" y="15" width="80" height="70" fill="none" stroke={color} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke"/>}
      {kind==="circle"&&<ellipse cx="50" cy="50" rx="39" ry="35" fill="none" stroke={color} strokeWidth={strokeWidth} vectorEffect="non-scaling-stroke"/>}
    </svg>
    <Handle className="project-shape-handle" type="target" position={Position.Left}/><Handle className="project-shape-handle" type="source" position={Position.Right}/>
  </div>;
}
...
const nodeTypes = { note: NoteNode, image: ImageNode, response: ResponseNode, doodle: DoodleNode, shape: ShapeNode, group: GroupNode };
...
function ProjectWorkspaceInner({project,rename,close}:{project:ProjectWorkspaceProject;rename:(title:string)=>void;close:()=>void}) {
  ...
  const selectedTextNodes=selectedNodes.filter(node=>node.type==="note");
  const selectedFontSize=Math.max(10,Math.min(48,Number(selectedTextNodes[0]?.data.fontSize)||14));
  ...
  const addNote=()=>setNodes(current=>[...current,hydrateNode({id:`note-${Date.now()}`,type:"note",position:{x:100+current.length*28,y:100+current.length*24},data:{title:"文字卡片",text:"",fontSize:14},style:{width:260,height:180}})]);
  ...
  const addShape=(shapeKind:ShapeKind)=>setNodes(current=>{
    const highestZ=Math.max(0,...current.map(node=>typeof node.zIndex==="number"?node.zIndex:0));
    const shape=hydrateNode({id:`shape-${shapeKind}-${Date.now()}`,type:"shape",position:{x:180+current.length*24,y:120+current.length*20},data:{title:`${shapeLabels[shapeKind]}图案`,shapeKind,shapeColor:"#d5a900",shapeStrokeWidth:3},style:{width:240,height:150},selected:true,zIndex:highestZ+1});
    return [...current.map(node=>node.selected?{...node,selected:false}:node),shape];
  });
  ...
  const renameSelected=()=>{const node=selectedNodes[0];if(!node)return;const target=document.querySelector<HTMLElement>(`.react-flow__node[data-id="${node.id}"] .project-node-rename`);target?.click()};
  const changeFontSize=(delta:number)=>{if(!selectedTextNodes.length)return;setNodes(current=>current.map(node=>{if(node.type!=="note"||!node.selected)return node;const next=Math.max(10,Math.min(48,(Number(node.data.fontSize)||14)+delta));return{...node,data:{...node.data,fontSize:next}}}))};
