...
const buildCanvasSvg = (project:ProjectWorkspaceProject,nodes:CanvasNode[],edges:Edge[]) => {
  ...
  const nodeMarkup=renderNodes.map(({node,position,width:nodeW,height:nodeH})=>{const point=offset(position),title=escapeXml(node.data.title||"未命名卡片"),color=node.data.groupColor||"#d5a900",headerHeight=node.type==="group"?38:34;
    ...
    const shapeColor=escapeXml(node.data.shapeColor||"#d5a900"),shapeStrokeWidth=Math.max(1,Math.min(12,Number(node.data.shapeStrokeWidth)||3)),shapeX=point.x+12,shapeY=point.y+headerHeight+12,shapeW=Math.max(24,nodeW-24),shapeH=Math.max(24,nodeH-headerHeight-24),shapeCenterY=shapeY+shapeH/2,shapeMarkup=node.type==="shape"?(node.data.shapeKind==="line"?`<line x1="${shapeX}" y1="${shapeCenterY}" x2="${shapeX+shapeW}" y2="${shapeCenterY}" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:node.data.shapeKind==="rectangle"?`<rect x="${shapeX}" y="${shapeY}" width="${shapeW}" height="${shapeH}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:node.data.shapeKind==="circle"?`<ellipse cx="${shapeX+shapeW/2}" cy="${shapeCenterY}" rx="${shapeW/2}" ry="${shapeH/2}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/>`:`<line x1="${shapeX}" y1="${shapeCenterY}" x2="${shapeX+shapeW-14}" y2="${shapeCenterY}" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}"/><path d="M ${shapeX+shapeW-22} ${shapeCenterY-9} L ${shapeX+shapeW-10} ${shapeCenterY} L ${shapeX+shapeW-22} ${shapeCenterY+9}" fill="none" stroke="${shapeColor}" stroke-width="${shapeStrokeWidth}" stroke-linecap="square" stroke-linejoin="miter"/>`):"";
    return`<g><rect x="${point.x}" y="${point.y}" width="${nodeW}" height="${nodeH}" fill="#fff" stroke="#202322" stroke-width="1.5"/><rect x="${point.x}" y="${point.y}" width="${nodeW}" height="${headerHeight}" fill="#f0f1ee" stroke="#c8cbc8" stroke-width="1"/><text x="${point.x+10}" y="${point.y+22}" fill="#202322" font-family="Arial, sans-serif" font-size="11" font-weight="700">${title}</text>${image}${node.type!=="image"&&node.type!=="shape"?body:""}${node.type==="response"?`<text x="${point.x+14}" y="${point.y+headerHeight+17}" fill="#202322" font-family="Arial, sans-serif" font-size="11">AI RESPONSE</text>`:""}${shapeMarkup}${strokes}</g>`;
  }).join("");
...
function ShapeNode({id,selected,data}:NodeProps<CanvasNode>) {
  const kind=data.shapeKind||"arrow",color=data.shapeColor||"#d5a900",strokeWidth=data.shapeStrokeWidth||3;
  return <NodeShell id={id} selected={selected} data={data}><div className="project-shape-node" aria-label={`${shapeLabels[kind]}图案`}>
